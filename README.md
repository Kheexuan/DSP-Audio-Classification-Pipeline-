...
# Conclusion
In conclusion, this project successfully developed an audio classification pipeline by combining digital signal processing techniques with machine learning-based classification. The preprocessing stage played an important role in improving the quality and consistency of the dataset through resampling, silence trimming, noise reduction, RMS normalisation, and DC offset correction. These steps helped remove irrelevant information while preserving the meaningful characteristics of the sound signals, making the data more suitable for feature extraction and classification.
...
Lastly,among the three evaluated models, Logistic Regression achieved the best overall performance, followed by Random Forest and SVM. This suggests that the extracted features were sufficiently informative and discriminative, allowing a simpler model to classify the sounds more effectively than the more complex alternatives.

Overall, the findings show that strong preprocessing and carefully selected features can have a major impact on classification performance, and that for this dataset, a simpler classifier was the most suitable choice.
